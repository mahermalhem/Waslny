...
const PinCode = ({route, navigation}) => {
  ...
  async function signInWithPhoneNumber(phoneNumber) {
    ...
    try {
      console.log(phoneNumber, 'in phoneNumber try');
      const confirmation = await auth().signInWithPhoneNumber(phoneNumber); //phoneNumber
      setConfirm(confirmation);
    } catch (error) {
      console.log(error);
      showToast(2, error.code, error.toString());
      // switch (error.code) {
      //   case 'auth/missing-client-identifier':
      //     showToast(2, strings('firebaseErrors.noPhoneUser'));
      //     break;
      // }
    }
  }
  ...
  return (
    <MainContainer>
      ...
      />
      <CText
        style={[{marginTop: wp(10), marginHorizontal: wp(1)}, style.text]}
        color={theme.colors.TEXT_TEXT_SECONDARY}>
        {strings('verify.EnterPinCodeRecivedOnYourPhone')}
      </CText>
      <OtpInputs
